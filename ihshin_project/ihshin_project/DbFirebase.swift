    ...
    func onChangingData(querySnapshot: QuerySnapshot?, error: Error?) {
        ...
        if querySnapshot.documentChanges.isEmpty {
            if let parentNotification = parentNotification {
                parentNotification(nil, nil) // 부모에게 알림
            }
            return
        }
        
        print(querySnapshot.documentChanges.count)
        ...
        for documentChange in querySnapshot.documentChanges {
            ...
            guard let dataDict = data["data"] as? [String: Any?] else {
                continue
            }
            
            let plan = Plan()
            plan.toPlan(dict: dataDict)
            ...
            if let parentNotification = parentNotification {
                parentNotification(plan, action) // 부모에게 알림
            }
        }
    }
